...
def get_gui() -> GUI:
    ...
    pack_element(hint_label)

    classes_search_row: Entry = Entry(window, justify="center", font=('Calibri', 14), width=25)
    cached_classes_value = get_cached_classes_value()
    classes_search_row.insert(index=0, string=cached_classes_value)
    pack_element(classes_search_row)

    start_button: Button = Button(window, text="Запуск", command=lambda: print('hello'), width=20, relief="groove", height=1)
...
def get_cached_classes_value() -> str:
...
def pack_element(element: GUI_ELEMENT) -> None:
