...
class Questioner:
    ...
    def _ask_about_url_mode(self):
        ...
        self._ask_user(master_page_parsed_classes=questionary.text("Введи классы элементов для парсинга на основной странице:", validate=bool))

        if self.user_answers.master_slave_mode == MasterSlaveMode.CLICK_INNER_TAG:
            self._ask_user(clicked_classes=questionary.text("Введи классы элементов, на которые нужно нажать для перехода на вторичные страницы:", validate=bool))

        if self.user_answers.master_slave_mode in (MasterSlaveMode.CLICK_MASTER_TAG, MasterSlaveMode.CLICK_INNER_TAG):
            self._ask_user(slave_page_parsed_classes=questionary.text("Введи классы элементов для парсинга на вторичных страницах:", validate=bool))

        self._ask_user(delay=questionary.text("Введи задержку между действиями:", validate=bool))

    def _ask_about_file_mode(self):
        self._ask_user(master_page_parsed_classes=questionary.text("Введи классы элементов для парсинга на странице:", validate=bool))
